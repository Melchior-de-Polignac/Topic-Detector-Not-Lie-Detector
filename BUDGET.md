...
| 2026-07-27 | **Rerun #1+#2 bundled** — H3 judge-bug recheck on `base`/`refusal_lora`/`refusal_lora+heretic` under the fixed judge, with Heretic at 200 trials (T2.3) so one run satisfies both items. Pod `6930bad2cb32` (user-rented A40); fresh — needs 14B download (~5min) + refusal adapter git-lfs pull + deps. Heretic 200-trial search is the long pole (~2h on A40). 8B judge calls on DeepInfra (cheap). | RunPod A40 ~$0.44/hr, ~3h est + DeepInfra micro | 3.0–3.5 | TBD | TBD |
